...
<div class="card card-primary">
    ...
    <form action="{{ route('menu-two-child.store') }}" method="post" enctype="multipart/form-data">
    @csrf
    @method('POST')
    <div class="card-body">
        <div class="form-group">
            ...
            <select id="id_menu_parent" class="form-control" name="id_menu_parent" style="cursor: pointer" required>
                ...
                <option value="{{ $MenuParentAdmin->id }}" {{ $MenuParentAdmin->id }}>{{$MenuParentAdmin->title}}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            <label for="exampleInputEmail1">Nama Menu Tengah</label>
            <select id="id_menu_one_child" class="form-control" name="id_menu_one_child" style="cursor: pointer" required>
                @foreach ($MenuOneChildAdmin as $MenuOneChildAdmin)
                <option value="{{ $MenuOneChildAdmin->id }}" {{ $MenuOneChildAdmin->id }}>{{$MenuOneChildAdmin->title}}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
    ...
    <div class="card-footer">
        <div class="btn-group btn-group-sm">
            <button type="button" class="btn btn-secondary" onclick=" window.location='{{ route('menu-two-child.index') }}' " >Kembali</button>
        </div>
        <div class="btn-group btn-group-sm">
