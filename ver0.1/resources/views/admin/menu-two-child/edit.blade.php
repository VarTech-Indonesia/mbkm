...
<div class="card card-primary">
    ...
    <form action="{{ route('menu-two-child.update',$item->id) }}" method="post" enctype="multipart/form-data">
    @csrf
    @method('put')
    <div class="card-body">
        ...
        <div class="form-group">
            ...
            <select id="id_menu_one_child" class="form-control" name="id_menu_one_child" style="cursor: pointer" required>
                ...
                <option value="{{ $MenuOneChildAdmin->id }}" {{ $MenuOneChildAdmin->id == $item->id_menu_one_child ? 'selected' : '' }}>{{$MenuOneChildAdmin->title}}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
